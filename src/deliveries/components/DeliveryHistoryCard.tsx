...
export default function DeliveryHistoryCard({ delivery }: DeliveryHistoryCardProps) {
  const statusClasses = getStatusPillClasses(delivery.status);
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 text-small text-muted-foreground">
            ...
            {delivery.selectedService && ` · ${delivery.selectedService}`}
          </p>
          {delivery.pricing && (
        ...
        <div className="text-right">
          <span
            className={`inline-flex rounded-full px-2.5 py-1 text-caption font-semibold ${statusClasses}`}
          >
            {delivery.statusLabel}
          </span>
          <p className="mt-3 text-small font-semibold text-accent">View delivery</p>
        </div>
      </div>
    </Link>
  );
